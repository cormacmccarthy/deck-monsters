...
class HitHarder extends HitCard {
	...
	getDamageRoll (player) {
		const damageRoll1 = roll({ primaryDice: this.damageDice, modifier: player.getBonus('str'), bonusDice: player.bonusAttackDice });
		const damageRoll2 = roll({ primaryDice: this.damageDice, modifier: player.getBonus('str'), bonusDice: player.bonusAttackDice });
