...
class Rehit extends HitCard {
	...
	hitCheck (player, target) {
		let attackRoll = roll({ primaryDice: this.attackDice, modifier: player.getBonus('dex'), bonusDice: player.bonusAttackDice, crit: true });
		let commentary = `Originally rolled ${attackRoll.naturalRoll.result}`;

		if (attackRoll.naturalRoll.result < 10) {
			attackRoll = roll({ primaryDice: this.attackDice, modifier: player.getBonus('dex'), bonusDice: player.bonusAttackDice, crit: true });
